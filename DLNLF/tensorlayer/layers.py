...
class BilinearLayer(Layer):
    def __init__(
        ...
        bcnn1 = tf.reshape(self.inputs[0], (-1, shape1[1] * shape1[2], shape1[3]))
        bcnn2 = tf.reshape(self.inputs[1], (-1, shape2[1] * shape2[2], shape2[3]))
...
class DenoiseLayer(Layer):
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            ...
            a_flat = tf.reshape(a, [tf.shape(a)[0], tf.shape(a)[1] * tf.shape(a)[2], tf.shape(a)[3]])
            b_flat = tf.reshape(b, [tf.shape(b)[0], tf.shape(b)[1] * tf.shape(b)[2], tf.shape(b)[3]])
            g_flat = tf.reshape(g, [tf.shape(g)[0], tf.shape(g)[1] * tf.shape(g)[2], tf.shape(g)[3]])
