...
class BilinearLayer(Layer):
    def __init__(
        ...
        bcnn1 = tf.reshape(self.inputs[0], (-1, shape1[1] * shape1[2] * shape1[3], shape1[4]))
        bcnn2 = tf.reshape(self.inputs[1], (-1, shape2[1] * shape2[2] * shape2[3], shape2[4]))
        bilinear = tf.matmul(bcnn1, bcnn2, transpose_a=True)

        # bcnn1 = tf.transpose(self.inputs[0], perm=[0, 3, 1, 2])
        # bcnn1 = tf.reshape(bcnn1, [-1, shape1[3], shape1[1] * shape1[2]])
        # bcnn2 = tf.reshape(self.inputs[1], [-1, shape1[1] * shape1[2], shape1[3]])
        # bilinear = tf.matmul(bcnn1, bcnn2)

        assert bilinear.get_shape()[1] == shape1[4]
        assert bilinear.get_shape()[2] == shape2[4]
        bilinear = tf.reshape(bilinear, (-1, shape1[4] * shape2[4]))
        ...
        # bilinear = tf.sqrt(tf.abs(bilinear) + 1e-12)
        self.outputs = tf.nn.l2_normalize(bilinear, dim=1)
...
class DenoiseLayer(Layer):
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            if embed:
                a = self.conv2d_same(self.inputs, depth, 1, strides=1, name='embA')
                b = self.conv2d_same(self.inputs, depth, 1, strides=1, name='embB')
            else:
                a, b = self.inputs, self.inputs
            g_orig = g = self.inputs
            if maxpool is not False and maxpool > 1:
                # b = tf.layers.mean_pooling3d(b, [maxpool, maxpool, maxpool], maxpool, padding='SAME')
                # g = tf.layers.max_pooling3d(g, [maxpool, maxpool, maxpool], maxpool, padding='SAME')
                b = tf.nn.avg_pool3d(b, maxpool, maxpool, padding='SAME')
                g = tf.nn.avg_pool3d(g, maxpool, maxpool, padding='SAME')
                # b = tf.nn.max_pool(b, ksize=[1, maxpool, maxpool, 1], strides=maxpool, padding='SAME')
                # g = tf.nn.max_pool(g, ksize=[1, maxpool, maxpool, 1], strides=maxpool, padding='SAME')

            # print("a: ",a.shape,"b: ",b.shape,"g: ",g.shape)
            a_shape = a.get_shape().as_list()
            b_shape = b.get_shape().as_list()
            g_shape = g.get_shape().as_list()
            a_flat = tf.reshape(a, [-1, a_shape[1] * a_shape[2] * a_shape[3], a_shape[4]])
            b_flat = tf.reshape(b, [-1, b_shape[1] * b_shape[2] * b_shape[3], b_shape[4]])
            g_flat = tf.reshape(g, [-1, g_shape[1] * g_shape[2] * g_shape[3], g_shape[4]])
            # print(a_flat.shape)
            ...
            #  print("f: ",f.shape,"a_fla: ",a_flat.shape,"b_flat: ",b_flat.shape,"g_flat: ",g_flat.shape)

            fg = tf.matmul(f, g_flat)
            # fg = tf.matmul(tf.transpose(g_flat, [0, 2, 1]), f)
            fg = tf.reshape(fg, tf.shape(g_orig))

            # with slim.arg_scope([slim.batch_norm], param_initializers={'gamma': tf.zeros_initializer()}):
            fg = self.conv2d_same(fg, self.inputs.get_shape().as_list()[-1], 1, strides=1, name='fgup')

            # sigma = tf.get_variable('sigma_ratio', [], initializer=tf.constant_initializer(0.0))
            outputs = act(tf.add(self.inputs, fg))
            print(outputs.shape)
            # outputs = tf.nn.relu(tf.add(self.inputs, fg))
    ...
    def conv2d_same(self, input_, output_dim, k_size=1, strides=1, name="conv2d"):
        W_init = tf.truncated_normal_initializer(stddev=0.02)
        # b_init = tf.constant_initializer(value=0.0)
        shape = [k_size, k_size, k_size, int(input_.get_shape()[-1]), output_dim]
        W_init_args = {}
        # b_init_args = {}
        with tf.variable_scope(name):
            ...
            conv = tf.nn.conv3d(input_, W, strides=[1, strides, strides, strides, 1], padding='SAME')
    ...
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            if embed:
                ...
                a, b = self.inputs, self.inputs
            g_orig = g = self.conv2d_same(self.inputs, depth, 1, strides=1, name='embG')
            if maxpool is not False and maxpool > 1:
                b = tf.layers.max_pooling3d(b, [maxpool, maxpool, maxpool], maxpool, padding='SAME')
                g = tf.layers.max_pooling3d(g, [maxpool, maxpool, maxpool], maxpool, padding='SAME')
                # b = tf.nn.max_pooling3d(b, ksize=[1, maxpool, maxpool, 1], strides=maxpool, padding='SAME')
                # g = tf.nn.max_pooling3d(g, ksize=[1, maxpool, maxpool, 1], strides=maxpool, padding='SAME')
            ...
            # with slim.arg_scope([slim.batch_norm], param_initializers={'gamma': tf.zeros_initializer()}):
            fg = self.conv2d_same(fg, self.inputs.shape[-1], 1, strides=1, name='fgup')
...
class NonlocalLayerNew(Layer):
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            a = self.conv2d_same(self.inputs, depth, 1, strides=1, name='embA')
            b = self.conv2d_same(self.inputs, depth, 1, strides=1, name='embB')
            g = self.conv2d_same(self.inputs, depth, 1, strides=1, name='embG')

            #   print("a: ",a.shape,"b: ",b.shape,"g: ",g.shape)
            a_flat = tf.reshape(a, [-1, a.shape[1]*a.shape[2], depth])
            b_flat = tf.reshape(b, [-1, g.shape[1]*b.shape[2], depth])
            g_flat = tf.reshape(g, [-1, a.shape[1]*g.shape[2], depth])

            # f = tf.nn.softmax(tf.matmul(a_flat, tf.transpose(b_flat, [0, 2, 1])))
            f = tf.nn.sigmoid(tf.matmul(a_flat, tf.transpose(b_flat, [0, 2, 1])))
            # f = tf.matmul(a_flat, tf.transpose(b_flat, [0, 2, 1]))

            #  print("f: ",f.shape,"a_fla: ",a_flat.shape,"b_flat: ",b_flat.shape,"g_flat: ",g_flat.shape)

            fg = tf.matmul(f, g_flat)
            fg = tf.reshape(fg, tf.shape(g))

            # with slim.arg_scope([slim.batch_norm], param_initializers={'gamma': tf.zeros_initializer()}):
            fg = self.conv2d_same(fg, self.inputs.shape[-1], 1, strides=1, name='fgup')
            sigma = tf.get_variable('sigma_ratio', [], initializer=tf.constant_initializer(0.0))
            # outputs = tf.nn.relu(tf.add(self.inputs, 250 * fg))
            outputs = tf.add(self.inputs, fg)
    ...
    # def conv2d(self, input_, output_dim, k_size=1, strides=1, stddev=0.02, name="conv2d"):
    #     with tf.variable_scope(name):
    ...
    def conv2d_same(self, input_, output_dim, k_size=1, strides=1, name="conv2d"):
        W_init = tf.truncated_normal_initializer(stddev=0.02)
        b_init = tf.constant_initializer(value=0.0)
        shape = [k_size, k_size, int(input_.get_shape()[-1]), output_dim]
        W_init_args = {}
        b_init_args = {}
        with tf.variable_scope(name):
            W = tf.get_variable(name='W_conv2d', shape=shape, initializer=W_init, **W_init_args)
            b = tf.get_variable(name='b_conv2d', shape=(shape[-1]), initializer=b_init, **b_init_args)
            conv = tf.nn.conv2d(input_, W, strides=strides, padding='SAME') + b
...
def SubpixelConv2d(net, scale=2, n_out_channel=None, act=tf.identity, name='subpixel_conv2d'):
    ...
    inputs = net.outputs
    inputs = tf.reshape(inputs, (-1, inputs.get_shape()[1], inputs.get_shape()[1], inputs.get_shape()[3] * scale))
...
def SubpixelConv2d_old(net, scale=2, n_out_channel=None, act=tf.identity, name='subpixel_conv2d'):
    ...
    def _phase_shift(I, r):
        ...
        X = tf.concat([tf.squeeze(x, axis=1) for x in X], 2)
        return tf.reshape(X, (bsize, a*r, b*1, 1))

    def _PS(X, r, n_out_channel):
        if n_out_channel > 1:
            assert int(X.get_shape()[-1]) == (r ** 1) * n_out_channel, _err_log
    ...
    if n_out_channel is None:
        assert int(inputs.get_shape()[-1]) / (scale ** 1) % 1 == 0, _err_log
        n_out_channel = int(int(inputs.get_shape()[-1]) / (scale ** 1))
